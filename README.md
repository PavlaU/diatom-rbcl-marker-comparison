...
Run the scripts from the repository root, so that the relative paths to `input/`, `NGS_raw/` and output folders are resolved correctly.

Raw FASTQ files should be placed locally in `NGS_raw/`. This folder is not included in the repository.

Before running `01_DADA_pipeline.r`, adjust the local path to Cutadapt:

```r
cutadapt_bin <- "/path/to/cutadapt-env/bin/cutadapt"
```

Run `01_DADA_pipeline.r` twice, once for each marker, by setting:

```r
marker_263 <- TRUE   # 263-bp marker
marker_263 <- FALSE  # 331-bp marker
```
## Scripts
